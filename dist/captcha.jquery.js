...
(function (factory) {
  ...
  class Captcha {
    ...
    loadImg(callback) {
      ...
      $.getJSON(_this.options.url.create).done(function (
        res,
        textStatus,
        jqXHR
      ) {
        if (res.code === 0) {
          ...
          _this.$captchaImg = _this.$captchaImgWrap
            .find("img")
            .attr("src", src)
    ...
    initMouse() {
      ...
      _this.$controlButton.on("mousedown." + libName, function (e) {
        if (
          ...
          _this.$controlButton.is(":animated")
        ) {
          return !1;
        }
      ...
      $(document).on("mouseup." + libName, function () {
        ...
        _this.$controlButton
          ...
          .removeClass("captcha-button-active");
    ...
    check() {
      ...
      $.getJSON(_this.options.url.check, {
        str: _this.runtime.str,
        angle: _this.runtime.deg,
      }).done(function (res) {
        // , token: this.token
        if (res && res.result) {
        ...
        _this.$control
          .addClass("captcha-control-horizontal")
          .one(
    ...
    move(x) {
      ...
      if (x < 0) {
        ...
        x >=
        $(_this.$control).width() - $(_this.$controlButton).outerWidth()
      ) {
        x = $(_this.$control).width() - $(_this.$controlButton).outerWidth();
      }

      let width =
        _this.options.size.control - $(_this.$controlButton).outerWidth();
      ...
      if (!isFail) {
        if (x < width + 1 && x > -1) {
          ...
          _this.$main.addClass("captcha-fail");
          _this.$controlButton
            .css({ transform: "translateX(210px)" })
            .removeClass("captcha-button-active");
        }
      }
    ...
    timerProgressBar(timer) {
      ...
      let timerProgressBar =
        _this.$elem[0].querySelectorAll(".captcha-timer-progress-bar")[0] ||
        null;
    ...
    insertCss(css) {
      if (!document.querySelectorAll("#J_captcha_css").length) {
        $("head").prepend(
          '<style id="J_captcha_css" type="text/css">' + css + "</style>"
        );
      }
    }
    ...
    captchaHTML(options) {
      return `<div id="${
        options.id
      }" class="captcha-root" style="--theme: ${options.theme};--progress-bar-color: ${options.timerProgressBarColor};--size-width: 305px;--size-img: 152px;--size-img-margin: 28px;--size-control: 275px;">
